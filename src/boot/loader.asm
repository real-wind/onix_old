...
print:
    mov ah, 0x0e; 设置 AH 寄存器，准备调用 BIOS 视频服务显示字符
.next:
    mov al, [si]
    cmp al, 0
    jz .done
...
read_disk:
    ...
    .reads:
        ...
        .readw:
            ...
            mov [edi], ax
            add edi, 2
            loop .readw; 每次loop读取一个字
        ret



; 段选择符_看Linux内核，段选择符示例
...
gdt_ptr: ;NULL 描述符: 8 字节 代码段描述符 (gdt_code): 8 字节 数据段描述符 (gdt_data): 8 字节 (gdt_end - gdt_base) = 24 字节
